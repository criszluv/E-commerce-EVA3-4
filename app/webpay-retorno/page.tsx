  ...
  useEffect(() => {
    ...
    async function procesarPago() {
      try {
        ...
        // Ignoramos error si es por duplicado (unique constraint) para evitar fallos al recargar
        ...
        // Esto marca la ORDEN como pagada/en curso.
        const { error: errorProp } = await supabase
            .from('proposals')
            .update({ status: 'accepted' })
            .eq('id', pedido.propuesta_id);

        if (errorProp) throw new Error(`Error actualizando propuesta: ${errorProp.message}`);

        // -------------------------------------------------------------------
        // ❌ PASO C ELIMINADO:
        // Ya NO actualizamos el 'projects' a 'in_progress'. 
        // El proyecto se queda 'open' para que otros clientes puedan comprarlo.
        // -------------------------------------------------------------------
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-gray-50 text-center font-sans">
      <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full border border-gray-100">
        ...
        <p className="text-gray-400 text-xs mt-8">ID Transacción: {token_ws || '...'}</p>
